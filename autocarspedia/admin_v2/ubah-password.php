...
if (isset($_POST['ubah'])) {
    ...
    if ($password_baru !== $konfirmasi_password_baru) {
        ...
        $query = "SELECT * FROM tbl_admin WHERE username = '$username' AND email = '$email'";
        $result = mysqli_query($db, $query);

        if (mysqli_num_rows($result) > 0) {
            // Update dengan password baru
            $password_baru_hashed = md5($password_baru); // Gunakan password_hash untuk keamanan lebih baik
            $update_query = "UPDATE tbl_admin SET password = '$password_baru_hashed' WHERE username = '$username' AND email = '$email'";
